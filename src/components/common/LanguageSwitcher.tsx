...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const checkAdminExists = async () => {
      try {
        const adminQuery = query(collection(db, "users"), where("role", "==", "admin"), where("status", "==", "active"));
        const querySnapshot = await getDocs(adminQuery);
        setHasAdminAccount(!querySnapshot.empty);
      } catch (error) {
        console.error("Error checking for admin account:", error);
        // Default to true to prevent locking out if Firestore is misconfigured
  ...
  return (
    <AuthContext.Provider value={{ user, role, loading, hasAdminAccount, loginWithUsername, logout, updateUserDisplayName, updateUserPassword }}>
      {children} 
