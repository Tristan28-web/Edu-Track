...
import { collection, doc, getDoc, query, orderBy, onSnapshot, where, getDocs, limit, Timestamp } from "firebase/firestore";
import { db } from "@/lib/firebase";
import type { AppUser, QuizResult, ResourceDownload, TeacherActivity } from "@/types";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Button } from "@/components/ui/button";
import { Loader2, AlertTriangle, User, BookOpenCheck, Library, Activity, Download, BookOpen } from "lucide-react";
...
export default function StudentProfilePage() {
  ...
  const handleDownloadPdf = async () => {
    ...
    if (!elementToCapture || !student) return;
    ...
    const fetchStudent = async () => {
      try {
        ...
        if (studentDoc.exists() && studentDoc.data().role === 'student') {
          setStudent({ id: studentDoc.id, ...studentDoc.data() } as AppUser);
        } else {
          throw new Error("Student not found or user is not a student.");
        }
      } catch (err: any) {
        throw new Error(`Failed to fetch student data: ${err.message}`);
      }
    };
    
    const quizzesQuery = query(collection(db, "users", studentId, "quizResults"), orderBy("submittedAt", "desc"));
    unsubscribes.push(onSnapshot(quizzesQuery, (snapshot) => {
        setQuizzes(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as any)));
    }));

    const resourcesQuery = query(collection(db, "resourceDownloads"), where("studentId", "==", studentId), orderBy("downloadedAt", "desc"));
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[calc(100vh-200px)]">
        ...
        <p className="ml-4 text-lg text-muted-foreground">Loading student profile...</p>
      </div>
    );
  }
  ...
  if (!student) {
    return <Alert><AlertTitle>Not Found</AlertTitle><AlertDescription>This student could not be found.</AlertDescription></Alert>
  }
        ...
        <Card className="shadow-lg print-hidden">
            <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                <div className="space-y-1">
                    <CardTitle className="text-3xl font-headline text-primary">{student.displayName}</CardTitle>
                    <CardDescription>@{student.username} &bull; Section: {student.sectionName || 'N/A'}</CardDescription>
                </div>
        ...
        <div ref={reportRef} className="bg-background p-4 rounded-lg">
            <div className="flex items-center gap-6 mb-6">
              <Avatar className="h-24 w-24 border">
                  ...
                  <AvatarFallback className="text-3xl">{getInitials(student.displayName)}</AvatarFallback>
              </Avatar>
              <div className="space-y-1">
                  <h2 className="text-2xl font-bold">{student.displayName}</h2>
                  <p className="text-md text-muted-foreground">@{student.username} &bull; Section: {student.sectionName || 'N/A'}</p>
              </div>
            </div>
            <div id="details-section">
            <Tabs defaultValue="quizzes" className="w-full">
                <TabsList className="grid w-full grid-cols-3 print-hidden">
                    <TabsTrigger value="quizzes">Quizzes Taken ({quizzes.length})</TabsTrigger>
                    <TabsTrigger value="resources">Downloaded Resources ({resources.length})</TabsTrigger>
                    <TabsTrigger value="reports">Activity ({activities.length})</TabsTrigger>
                </TabsList>
                
                <TabsContent value="quizzes" className="mt-6">
                    <Card>
                        <CardHeader><CardTitle>Taken Quizzes</CardTitle></CardHeader>
                        <CardContent>
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                ...
                                <TableBody>
                                    {quizzes.length > 0 ? quizzes.map(quiz => (
                                        <TableRow key={quiz.id}>
                                            <TableCell className="font-medium">{(quiz as any).quizTitle || 'Quiz'}</TableCell>
                                            <TableCell>{getTopicTitle(quiz.topic)}</TableCell>
                                            <TableCell>{quiz.percentage}%</TableCell>
                                            <TableCell>{format(quiz.submittedAt.toDate(), 'PP')}</TableCell>
                                        </TableRow>
                                    )) : (
                                        <TableRow><TableCell colSpan={4} className="text-center h-24">No quizzes taken by this student.</TableCell></TableRow>
                                    )}
                                </TableBody>
                            </Table>
                        </CardContent>
                ...
                <TabsContent value="resources" className="mt-6">
                    <Card>
                        <CardHeader><CardTitle>Downloaded Resources</CardTitle></CardHeader>
                        <CardContent>
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                ...
                                <TableBody>
                                    {resources.length > 0 ? resources.map(res => (
                                        <TableRow key={res.id}>
                                            <TableCell className="font-medium flex items-center gap-2"><BookOpen className="h-4 w-4 text-muted-foreground"/> {res.resourceId}</TableCell>
                                            <TableCell>{format(res.downloadedAt.toDate(), 'PP p')}</TableCell>
                                        </TableRow>
                                    )) : (
                                        <TableRow><TableCell colSpan={2} className="text-center h-24">No resources downloaded by this student.</TableCell></TableRow>
                                    )}
                                </TableBody>
                            </Table>
                        </CardContent>
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {paginatedActivities.length > 0 ? paginatedActivities.map(activity => (
                                        ...
                                        <TableRow><TableCell colSpan={2} className="text-center h-24">No recent activity from this student in the selected time range.</TableCell></TableRow>
                                    )}
                                </TableBody>
                            </Table>
                        </CardContent>
