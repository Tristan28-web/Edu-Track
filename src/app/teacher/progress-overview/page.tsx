...
export default function TeacherProgressOverviewPage() {
  ...
  useEffect(() => {
    if (!authUser || authUser.role !== 'teacher') {
      setIsLoading(false);
      if (authUser) setError("Accessing this page requires a teacher login.");
      return;
    }
    ...
    const unsubscribeStudents = onSnapshot(studentsQuery, (studentsSnapshot) => {
        setStudents(studentsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as AppUser)));
        if (!sections.length) {
             const sectionsQuery = query(collection(db, "sections"), where("teacherId", "==", authUser.id), orderBy("name"));
    ...
    <div className="space-y-8">
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Select a student to view their detailed academic performance report.
          </CardDescription>
        </CardHeader>
      </Card>
                    ...
                    filteredStudents.map(student => (
                      <TableRow key={student.id}>
                        <TableCell>
                            ...
                            <p className="text-xs text-muted-foreground sm:hidden">@{student.username}</p>
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          <Button asChild variant="outline" size="sm">
                            <Link href={`/teacher/progress-overview/${student.id}`}>
